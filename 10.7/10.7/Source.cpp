...
int main(){
	...
	string v_user = user, simvol = user;
	...
	cout << "Количество символов в строке " << simvol.size() << endl;
...
void rifm(string v_user)
{
	...
	for (int q = 2; s[q] != '\0'; q++)
		...
		if (s[q] == ' ') { okonchani = ""; okonchani.push_back(s[q - 1]); okonchani.push_back(s[q - 2]); odt = q;   vsp2 = "";
			for (int w = q - 1; s[w] != ' '; w--) { vsp2.push_back(s[w]); }
				reverse(vsp2.begin(), vsp2.end());  reverse(okonchani.begin(), okonchani.end());
					cout << s[q] << endl;

			for (int r = odt; s[r] != '\0'; r++) {
				if (s[r] == okonchani[0] && s[r + 1] == okonchani[1] && s[r + 2] == char(32)) {
					for (int t = r + 1; s[t] != char(32); t--) vsp3.push_back(s[t]); 
					   reverse(vsp3.begin(), vsp3.end()); 
							cout <<"\t"<< vsp2 << "\t\t" << vsp3 << endl; okonchani = ""; vsp2 = ""; vsp3 = "";}}}} 
